...
class Engine:
    ...
    def run(self,cid,odir,prog,log):
        ...
        for i in range(start,total):
            ...
            if not sls: s["f"]+=1; prog(i+1,total); self._svc(ck,i+1); continue
            im=[]
            for j,u in enumerate(sls):
                if self.stop: self._ck[cid]=i; self._svc(ck,i); return
                while self.pause and not self.stop: time.sleep(0.3)
                img=self.dlimg(u)
                if img: im.append(img)
                if j%3==0: prog(i+1,total,f"{nm[:30]} {j+1}/{len(sls)}")
            if im and self.pdf(im,out):
                cache[pid]=out
                if fp: self._svh(hf_,pid,fp); hc[fp]=(pid,out)
                self._sv(cf,cache); s["n"]+=1; log(f"[完成] {nm} ({len(im)}页)")
            else: s["f"]+=1
            prog(i+1,total); self._svc(ck,i+1); time.sleep(0.15)
